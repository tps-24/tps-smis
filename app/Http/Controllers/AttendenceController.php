...
class AttendenceController extends Controller
{
    ...
    public function store(Request $request, $type, $platoon_id)
    {
        ...
        if (!$todayRecords->isEmpty()) {
            return redirect()->to('attendences/type/1')->with('error', "Attendences for this platoon already recorded.");
        }
    ...
    {        $selectedSessionId = session('selected_session');
        if (!$selectedSessionId)
            $selectedSessionId = 1;
        return $platoon->safari()->where('session_programme_id', $selectedSessionId)->pluck('student_id')->toArray();
    }
}
