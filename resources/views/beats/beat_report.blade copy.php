...
@section('content')
    ...
    <div class="container">
        ...
        <form action="{{ route('report.generate') }}" method="get" class="mb-4">
            ...
            <a href="{{ route('report.download', ['start_date' => request('start_date'), 'end_date' => request('end_date'), 'date_filter' => request('date_filter')]) }}" class="btn btn-secondary mt-3">Download PDF</a>
        </form>
        ...
        <ul class="nav nav-tabs" id="companyTabs" role="tablist">
            @foreach ($report['companies'] as $index => $company)
                <li class="nav-item" role="presentation">
                    <button class="nav-link {{ $index == 0 ? 'active' : '' }}" id="tab-{{ $company['company_id'] }}" data-bs-toggle="tab" data-bs-target="#content-{{ $company['company_id'] }}" type="button" role="tab" aria-controls="content-{{ $company['company_id'] }}" aria-selected="{{ $index == 0 ? 'true' : 'false' }}">
        ...
        <div class="tab-content" id="companyTabsContent">
            @foreach ($report['companies'] as $index => $company)
                <div class="tab-pane fade {{ $index == 0 ? 'show active' : '' }}" id="content-{{ $company['company_id'] }}" role="tabpanel" aria-labelledby="tab-{{ $company['company_id'] }}">
                    <ul>
