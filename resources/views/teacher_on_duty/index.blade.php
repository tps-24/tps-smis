    ...
    <div class="d-flex justify-content-center">
        ...
        <form class="d-flex" action="{{route('teacher_on_duty.search')}}" method="GET">
            ...
            <div class="d-flex gap-2">
                ...
                <!-- Company Dropdown -->
                <select onchange="this.form.submit()" class="form-select me-2" name="company_id" required>
...
<h4>Teachers on Duty</h4>
<div class="card-body">
    <div class="table-outer mt-4">
        ...
        <table class="table table-striped truncate m-0">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{++$j}}</td>
                    <td>{{ $teacher->staff->forceNumber }}</td>
                    <td>{{ $teacher->staff->firstName }} {{ $teacher->staff->middleName }}
                        {{ $teacher->staff->lastName }}</td>
                    <td>{{ $teacher->staff->rank }}</td>
                            ...
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title text-info" id="statusModalLabel{{  $teacher->id ?? ''}}">
                                        Unassign {{ $teacher->staff->forceNumber }} {{ $teacher->staff->rank }} {{ $teacher->staff->user->name }}
                                    </h5>
                                ...
                                <div class="modal-body">
                                    ...
                                    <form action="{{ route('teacher_on_duty.unassign', $teacher->id) }}" method="GET">
                                        @csrf
                                        <div class="mb-3">
                ...
                <tbody>
                    ...
                    <div class="modal fade" id="statusModal{{  $staff->id ?? '' }}" tabindex="-1"
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title text-info" id="statusModalLabel{{  $staff->id ?? ''}}">
                                        Assign {{ $staff->forceNumber }} {{ $staff->rank }} {{ $staff->user->name }}
                                    </h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                        aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <form action="{{ route('teacher_on_duty.store', $staff->id) }}" method="GET">
                                        @csrf
                                        <div class="mb-3">
