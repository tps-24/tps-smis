    ...
    <form method="POST" action="{{ route('patrol-areas.update', $patrolArea) }}">
        ...
        <select name="beat_exception_ids[]" id="beat_exception_ids" multiple>
            @foreach ($beatExceptions as $beatException)
                <option value="{{ $beatException->id }}" @if(in_array($beatException->id, json_decode($patrolArea->beat_exception_ids, true) ?? [])) selected @endif>
                    {{ $beatException->name }}
                </option>
            @endforeach
        </select>
        ...
        <select name="beat_time_exception_ids[]" id="beat_time_exception_ids" multiple>
            @foreach ($beatTimeExceptions as $beatTimeException)
                <option value="{{ $beatTimeException->id }}" @if(in_array($beatTimeException->id, json_decode($patrolArea->beat_time_exception_ids, true) ?? [])) selected @endif>
