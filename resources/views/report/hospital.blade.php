    ...
    <div class="container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a href="#">Hospital</a></li>
            </ol>
        </nav>
    </div>
</nav>
...
<div class="d-flex justify-content-end">
    <a href="{{route('reports.generateHospitalReport')}}" title="Download report" class="btn btn-sm btn-success"><i class="gap 2 bi bi-download"></i>
        Report</a>
</div>
<div class="mb-4">
...
@foreach ($mostOccurredStudents as $patient)
    <tr class="txt-danger" style="color:red">
            ...
            <td style="{{ $patient['count'] > 2 ? 'color:red' : '' }}">{{ $patient['student']->force_number?? '' }} {{ $patient['student']->first_name }} {{ $patient['student']->last_name }}</td>
            <td style="{{ $patient['count'] > 2 ? 'color:red' : '' }}">{{ $patient['student']->company->name }} - {{ $patient['student']->platoon }}</td>
