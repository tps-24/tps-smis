...
<div class="app-header d-flex align-items-center">
    ...
    // Check if a session ID has been submitted 
        if (request()->has('session_id')) { 
            // Store the selected session ID in the sessionActive table 
            DB::table('session_active')->updateOrInsert( 
                ['key' => 'selected_session'], 
                ['value' => request()->session_id, 'updated_at' => now()] 
            ); 

            // Update the selected session ID in the session_programmes table 
            DB::table('session_programmes')->where('id', request()->session_id)->update( 
                ['is_active' => 1, 'updated_at' => now()] 
            ); 
            
            // Set is_active to 0 for all other sessions 
            DB::table('session_programmes')->where('id', '!=', request()->session_id)->update( 
        ...
        // Get the selected session ID from the sessionActive table 
        $selectedSessionId = DB::table('session_active')->where('key', 'selected_session')->value('value'); 
