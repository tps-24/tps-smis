...
Artisan::command('app:daily-beats', function () {
    ...
    // Fetch all companies with areas
    $companies = Company::with('areas')->get(); // Ensure areas are loaded
    ...
    foreach ($companies as $company) {
        ...
        // Ensure areas are not null before looping
        if (!is_null($company->areas) && is_iterable($company->areas)) {
            foreach ($company->areas as $area) {
                $this->info('Student company');
                $beatController->store(1, $area->id, NULL, 1, $company->id, "18", "00");
            }
        } else {
            $this->warn("⚠️ No areas found for company: {$company->name}");
        }
    }
})->describe('Generate beats for the next day.');
